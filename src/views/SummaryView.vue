<template>
  <main class="summary flex-center flex-col">
    ...
    <div class="place-self-center w-1/2 bg-indigo-200 border-indigo-800 rounded-full h-2.5 mb-4">
      <div
        class="bg-orange-500 h-2.5 rounded-full"
        :style="progressBar(counter)"
      ></div>
    </div>
    <details class="rounded-lg p-2 shadow-[2px_2px] shadow-orange-500 border-indigo-800 border-4 flex flex-col">
      <summary>Click to check your answers</summary>
      <p class="flex" v-for="(answer, index) in myAnswers" :key="index" :class="{wrong:!isCorrectArray}">
        
        <span v-if="isCorrectArray[index]"> ✅ </span>
        <span v-else-if="!isCorrectArray[index]"
          >❌
          <!-- {{ decodeURIComponent(correctAnswers[index]) }} -->
      </span>{{ decodeURIComponent(answer) }}
      </p>
    </details>
    <router-link
...
const { myAnswers, correctAnswers } = storeToRefs(useQuizStore());
const counter = ref(0);
